...
jobs:
  job1:
    ...
    it("reports queue as unsupported when the feature is disabled", async () => {
      const input = `
on: push
concurrency:
  group: deploy
...
jobs:
  job1:
    ...
    steps:
      ...
      const unsupportedQueueErrors = result.filter(d => d.message === "Unexpected value 'queue'");
      expect(unsupportedQueueErrors).toHaveLength(1);
